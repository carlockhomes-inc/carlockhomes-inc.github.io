/**
 * CarlockHomes Recruitment Form - GAS Backend (v18.9-RECAPTCHA-FINAL)
 * 最新のreCAPTCHA v3鍵で、不正なBotアクセスを完璧にブロックします。
 * これをまるごとコピーして貼り付け、再度「新しいデプロイ」をしてください。
 */
...
function doPost(e) {
  ...
  try {
    ...
    if (p.access_token !== EXPECTED_TOKEN) {
      logAudit(timestamp, "INVALID_TOKEN", "トークン不一致", p);
    ...
    if (cache.get(rateKey)) {
      logAudit(timestamp, "RATE_LIMIT", "制限超過", p);
      return responseError("Too Many Requests", 429);
    }
    cache.put(rateKey, "1", TOKEN_EXPIRY_SEC);

    // 3. Honeypot Check
    if ((p.company_verify && p.company_verify !== "") || (p.office_confirm && p.office_confirm !== "")) {
      logAudit(timestamp, "HONEYPOT_TRIGGERED", "Bot検知(Honeypot)", p);
      return responseError("Bot Detected", 400);
    }

    // 4. reCAPTCHA v3 厳格検証
    const reToken = p.recaptcha_token || "";
    if (!reToken || ["TIMEOUT_OR_BLOCKED", "NOT_LOADED", "EXECUTE_ERROR", "CATCH_ERROR"].includes(reToken)) {
      logAudit(timestamp, "BAD_RECAPTCHA_TOKEN", "判定不能（鍵エラー等）: " + reToken, p);
      return responseError("Verification Blocked", 400);
    }

    const secretKey = "6Lfi6MEsAAAAAGPFmJLhXIvA9u6h_E8CPvQAJDLp";
    ...
    if (!verifyData.success) {
      logAudit(timestamp, "RECAPTCHA_FAIL", "Google検証エラー: " + JSON.stringify(verifyData['error-codes']), p);
    ...
    if (verifyData.hostname !== ALLOWED_HOSTNAME && verifyData.hostname !== "localhost" && verifyData.hostname) {
      logAudit(timestamp, "DOMAIN_MISMATCH", "拒絶ドメイン: " + verifyData.hostname, p);
      return responseError("Invalid Domain", 400);
    }
    
    if (typeof verifyData.score === 'number' && verifyData.score < 0.3) {
      logAudit(timestamp, "LOW_SCORE", "スコア不足: " + verifyData.score, p);
      return responseError("Security Score Too Low", 400);
    }
    ...
    let rowData = [];
    if (type === 'entry') {
      rowData = [timestamp, clean(p.name), clean(p.age), clean(p.contact), clean(p.workStyle), clean(p.pr), clean(p.source), "V18.9-FINAL"];
    } else {
      rowData = [timestamp, clean(p.name), clean(p.contactType), clean((p.tel ? p.tel + " / " : "") + (p.email || "")), clean(p.body), clean(p.source), "V18.9-FINAL"];
    }
    sheet.appendRow(rowData);
    
    logAudit(timestamp, "SUCCESS", "正常に完了", p);
    sendEmails(p, type);
    
    return responseSuccess("Submission Successful");
...
function sendEmails(p, type) {
  try {
    const adminSubject = `【CLH通知】${type === 'entry' ? 'エントリー' : 'お問い合わせ'} (${p.name}様)`;
