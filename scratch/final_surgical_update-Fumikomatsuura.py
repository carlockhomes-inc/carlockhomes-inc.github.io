...
# 2. Ultra-flexible replacements
# Badge (searching by the specific style content)
content = re.sub(
    r'(style="display:inline-block;background:#fff;color:#FF4444;padding:6px 20px;border-radius:100px;font-size:12px;font-weight:900;margin-bottom:20px;">).*?(</div>)',
    r'\1TOPICS\2',
    content,
    flags=re.DOTALL
)

# Hero Title (searching by the font-size:36px marker)
content = re.sub(
...
# Hero Paragraph (searching by opacity:.95)
content = re.sub(
...
# Yellow Label (opacity:.8)
content = re.sub(
    r'(<div.*?font-size:13px;font-weight:900;opacity:\.8;.*?>).*?(</div>)',
    r'\1NEWS\2',
    content,
    flags=re.DOTALL
)

# Yellow Value (font-size:32px)
content = re.sub(
    r'(<div.*?font-size:32px;font-weight:900;.*?>).*?(</div>)',
